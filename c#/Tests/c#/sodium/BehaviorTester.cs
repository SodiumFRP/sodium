...
namespace Tests.sodium
{
  ...
  public class BehaviorTester
  {
    ...
    {
      var a = new BehaviorSink<int>(1);
      Behavior<int> a3 = a.Map(x => x * 3);
      Behavior<int> a5 = a.Map(x => x * 5);
    ...
    public void TestLoopBehavior()
    {
      ...
      Behavior<int> sum_out = Transaction.Run(() =>
      {
        ...
        Behavior<int> sumOut = ea.Snapshot(sum, (x, y) => x + y).Hold(0);
        sum.Loop(sumOut);
        return sumOut;
      });
      var @out = new List<int>();
      ...
      Assert.That(sum_out.Sample(), Is.EqualTo(6));
    }
