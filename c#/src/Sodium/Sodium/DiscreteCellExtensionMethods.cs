...
namespace Sodium
{
    public static class DiscreteCellExtensionMethods
    {
        ...
        public static DiscreteCell<TResult> Lift<T, TResult>(this IEnumerable<DiscreteCell<T>> c, Func<IReadOnlyList<T>, TResult> f) => c.ToArray().Lift(f);
        ...
        /// <param name="f">The binary function to lift into the cells.</param>
        /// <returns>A cell containing values resulting from the function applied to the input cells' values.</returns>
        public static DiscreteCell<TResult> Lift<T, TResult>(this IReadOnlyCollection<DiscreteCell<T>> c, Func<IReadOnlyList<T>, TResult> f) => new DiscreteCell<TResult>(c.Select(i => i.Cell).Lift(f));
