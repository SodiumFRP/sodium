...
function lookup(url, sRequest) {
    var sResponse = Rx.Observable.create(function (observer) {
        return sRequest.subscribe(function(req) {
            ...
            script.src = url+encodeURIComponent(req) + "&callback=jsonpCallbacks." + fnName;
...
function autocomplete(textEdit) {
    ...
    var sKeyPresses = Rx.Observable.fromEvent(textEdit, 'keyup'),
        editText = sKeyPresses.map(function (e) {return textEdit.value; }).startWith(textEdit.value),
        sDebounced = sKeyPresses.startWith(null).debounce(100),
        sTextUpdate = changes(sDebounced.withLatestFrom(editText, function (key, text) {
            return text;
        }));
    ...
    var lookedUp = lookup("http://gd.geobytes.com/AutoCompleteCity?q=",
        sTextUpdate.merge(sTabKey.withLatestFrom(editText, function (key, text) {
            ...
            var req = req_resp[0],
                resp = req_resp[1];
            return resp.length == 1 && (resp[0] == "%s" || resp[0] == "" || resp[0] == req) ? null : resp;
        }).merge(sEscapeKey.merge(sEnterKey).merge(sClicked).map(null)).startWith(null);
    ...
    lookedUp.subscribe(function(items) {
        if (items !== null) {
            ...
            if (popup.style.display != 'block') {
                ...
                popup.style.top = textEdit.offsetTop + textEdit.offsetHeight;
